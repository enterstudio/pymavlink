...
if (MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS) {
	Array.Copy(BitConverter.GetBytes(batVolt),0,msg,0,sizeof(UInt16));
	Array.Copy(BitConverter.GetBytes(sensLoad),0,msg,2,sizeof(byte));
	Array.Copy(BitConverter.GetBytes(ctrlLoad),0,msg,3,sizeof(byte));
...
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[4];
	_mav_put_UInt16(buf, 0, batVolt);
	_mav_put_byte(buf, 2, sensLoad);
	_mav_put_byte(buf, 3, ctrlLoad);
