...
public static UInt16 mavlink_msg_command_long_pack(byte system_id, byte component_id, byte[] msg,
                               byte target_system, byte target_component, byte command, byte confirmation, Single param1, Single param2, Single param3, Single param4, Single param5, Single param6, Single param7)
{
if (MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS) {
	...
	Array.Copy(BitConverter.GetBytes(param5),0,msg,16,sizeof(Single));
	Array.Copy(BitConverter.GetBytes(param6),0,msg,20,sizeof(Single));
	Array.Copy(BitConverter.GetBytes(param7),0,msg,24,sizeof(Single));
	Array.Copy(BitConverter.GetBytes(target_system),0,msg,28,sizeof(byte));
	Array.Copy(BitConverter.GetBytes(target_component),0,msg,29,sizeof(byte));
	Array.Copy(BitConverter.GetBytes(command),0,msg,30,sizeof(byte));
	Array.Copy(BitConverter.GetBytes(confirmation),0,msg,31,sizeof(byte));

} else {
    mavlink_command_long_t packet = new mavlink_command_long_t();
