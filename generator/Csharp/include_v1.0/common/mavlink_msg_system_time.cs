...
static uint16 mavlink_msg_system_time_pack(byte system_id, byte component_id, ref byte[] msg,
                               UInt64 public time_usec, UInt32 public time_boot_ms)
{
...
static inline void mavlink_msg_system_time_send(mavlink_channel_t chan, UInt64 public time_usec, UInt32 public time_boot_ms)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[12];
	_mav_put_UInt64(buf, 0, time_usec);
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_SYSTEM_TIME, buf, 12, 143);
#else
    mavlink_system_time_t packet;
	packet.time_usec = time_usec;
	packet.time_boot_ms = time_boot_ms;

    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_SYSTEM_TIME, (const char *)&packet, 12, 143);
#endif
}
