...
public partial class Mavlink
{
    ...
    public struct mavlink_nav_controller_output_t
    {
     ...
     public  Int16 target_bearing; /// Bearing to current MISSION/target in degrees
     public  UInt16 wp_dist; /// Distance to active MISSION in meters
...
if (MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS) {
	...
	Array.Copy(BitConverter.GetBytes(alt_error),0,msg,8,sizeof(Single));
	Array.Copy(BitConverter.GetBytes(aspd_error),0,msg,12,sizeof(Single));
	Array.Copy(BitConverter.GetBytes(xtrack_error),0,msg,16,sizeof(Single));
	Array.Copy(BitConverter.GetBytes(nav_bearing),0,msg,20,sizeof(Int16));
	Array.Copy(BitConverter.GetBytes(target_bearing),0,msg,22,sizeof(Int16));
	Array.Copy(BitConverter.GetBytes(wp_dist),0,msg,24,sizeof(UInt16));
