...
if (MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS) {
	Array.Copy(BitConverter.GetBytes(gain),0,msg,0,sizeof(Single));
	Array.Copy(BitConverter.GetBytes(interval),0,msg,4,sizeof(UInt16));
	Array.Copy(BitConverter.GetBytes(exposure),0,msg,6,sizeof(UInt16));
	Array.Copy(BitConverter.GetBytes(cam_no),0,msg,8,sizeof(byte));
	Array.Copy(BitConverter.GetBytes(cam_mode),0,msg,9,sizeof(byte));
	Array.Copy(BitConverter.GetBytes(trigger_pin),0,msg,10,sizeof(byte));
...
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[11];
	_mav_put_Single(buf, 0, gain);
	_mav_put_UInt16(buf, 4, interval);
	_mav_put_UInt16(buf, 6, exposure);
	_mav_put_byte(buf, 8, cam_no);
	_mav_put_byte(buf, 9, cam_mode);
	_mav_put_byte(buf, 10, trigger_pin);
