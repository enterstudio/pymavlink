...
{
 Single extra_value; ///< Correspondent value to given extra_param
 UInt16 shutter_speed; ///< Divisor number //e.g. 1000 means 1/1000 (0 means ignore)
 byte target_system; ///< System ID
 byte target_component; ///< Component ID
 byte mode; ///< Mode enumeration from 1 to N //P, TV, AV, M, Etc (0 means ignore)
 byte aperture; ///< F stop number x 10 //e.g. 28 means 2.8 (0 means ignore)
 byte iso; ///< ISO enumeration from 1 to N //e.g. 80, 100, 200, Etc (0 means ignore)
 byte exposure_type; ///< Exposure type enumeration from 1 to N (0 means ignore)
 byte command_id; ///< Command Identity (incremental loop: 0 to 255)//A command sent multiple times will be executed or pooled just once
 byte engine_cut_off; ///< Main engine cut-off time before camera trigger in seconds/10 (0 means no cut-off)
 byte extra_param; ///< Extra parameters enumeration (0 means ignore)
...
static inline uint16_t mavlink_msg_digicam_configure_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       byte target_system, byte target_component, byte mode, UInt16 shutter_speed, byte aperture, byte iso, byte exposure_type, byte command_id, byte engine_cut_off, byte extra_param, Single extra_value)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[15];
	_mav_put_Single(buf, 0, extra_value);
	_mav_put_UInt16(buf, 4, shutter_speed);
	_mav_put_byte(buf, 6, target_system);
	_mav_put_byte(buf, 7, target_component);
	_mav_put_byte(buf, 8, mode);
	_mav_put_byte(buf, 9, aperture);
	_mav_put_byte(buf, 10, iso);
	_mav_put_byte(buf, 11, exposure_type);
	_mav_put_byte(buf, 12, command_id);
	_mav_put_byte(buf, 13, engine_cut_off);
	_mav_put_byte(buf, 14, extra_param);
...
static inline void mavlink_msg_digicam_configure_send(mavlink_channel_t chan, byte target_system, byte target_component, byte mode, UInt16 shutter_speed, byte aperture, byte iso, byte exposure_type, byte command_id, byte engine_cut_off, byte extra_param, Single extra_value)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[15];
	_mav_put_Single(buf, 0, extra_value);
 ...
 * @return System ID
 */
static inline byte mavlink_msg_digicam_configure_get_target_system(const mavlink_message_t* msg)
{
 ...
 * @return Component ID
 */
static inline byte mavlink_msg_digicam_configure_get_target_component(const mavlink_message_t* msg)
{
 ...
 */
static inline byte mavlink_msg_digicam_configure_get_mode(const mavlink_message_t* msg)
{
 ...
 */
static inline UInt16 mavlink_msg_digicam_configure_get_shutter_speed(const mavlink_message_t* msg)
{
 ...
 */
static inline byte mavlink_msg_digicam_configure_get_aperture(const mavlink_message_t* msg)
{
 ...
 */
static inline byte mavlink_msg_digicam_configure_get_iso(const mavlink_message_t* msg)
{
	return _MAV_RETURN_byte(msg,  10);
}
 ...
 */
static inline byte mavlink_msg_digicam_configure_get_exposure_type(const mavlink_message_t* msg)
{
 ...
 */
static inline byte mavlink_msg_digicam_configure_get_command_id(const mavlink_message_t* msg)
{
 ...
 */
static inline byte mavlink_msg_digicam_configure_get_engine_cut_off(const mavlink_message_t* msg)
{
 ...
 */
static inline byte mavlink_msg_digicam_configure_get_extra_param(const mavlink_message_t* msg)
{
 ...
 */
static inline Single mavlink_msg_digicam_configure_get_extra_value(const mavlink_message_t* msg)
{
