...
{
 UInt16 Vcc; ///< board voltage (mV)
 byte I2Cerr; ///< I2C error count
} mavlink_hwstatus_t;
...
static inline uint16_t mavlink_msg_hwstatus_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       UInt16 Vcc, byte I2Cerr)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[3];
	_mav_put_UInt16(buf, 0, Vcc);
	_mav_put_byte(buf, 2, I2Cerr);
...
static inline void mavlink_msg_hwstatus_send(mavlink_channel_t chan, UInt16 Vcc, byte I2Cerr)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[3];
	_mav_put_UInt16(buf, 0, Vcc);
 ...
 */
static inline UInt16 mavlink_msg_hwstatus_get_Vcc(const mavlink_message_t* msg)
{
 ...
 */
static inline byte mavlink_msg_hwstatus_get_I2Cerr(const mavlink_message_t* msg)
{
