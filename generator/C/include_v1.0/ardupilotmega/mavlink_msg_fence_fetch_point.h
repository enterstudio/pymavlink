...
#define MAVLINK_MSG_ID_FENCE_FETCH_POINT_LEN 1
...
#define MAVLINK_MESSAGE_INFO_FENCE_FETCH_POINT { \
	"FENCE_FETCH_POINT", \
	1, \
	{  { "idx", NULL, MAVLINK_TYPE_UINT8_T, 0, 0, offsetof(mavlink_fence_fetch_point_t, idx) }, \
         } \
}
						       ...
						       uint8_t idx)
{
	...
	return mavlink_finalize_message(msg, system_id, component_id, 1, 165);
}
						           ...
						           uint8_t idx)
{
	...
	msg->msgid = MAVLINK_MSG_ID_FENCE_FETCH_POINT;
	return mavlink_finalize_message_chan(msg, system_id, component_id, chan, 1, 165);
}
...
{
	return mavlink_msg_fence_fetch_point_pack(system_id, component_id, msg, fence_fetch_point->idx);
}
...
static inline void mavlink_msg_fence_fetch_point_send(mavlink_channel_t chan, uint8_t idx)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[1];
	_mav_put_uint8_t(buf, 0, idx);

	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_FENCE_FETCH_POINT, buf, 1, 165);
#else
	mavlink_fence_fetch_point_t packet;
	packet.idx = idx;

	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_FENCE_FETCH_POINT, (const char *)&packet, 1, 165);
#endif
}
...
#else
	memcpy(fence_fetch_point, _MAV_PAYLOAD(msg), 1);
#endif
}
