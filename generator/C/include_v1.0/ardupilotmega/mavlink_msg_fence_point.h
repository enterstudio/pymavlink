...
#define MAVLINK_MSG_ID_FENCE_POINT_LEN 10
...
#define MAVLINK_MESSAGE_INFO_FENCE_POINT { \
	...
	{  { "lat", NULL, MAVLINK_TYPE_FLOAT, 0, 0, offsetof(mavlink_fence_point_t, lat) }, \
         ...
         { "idx", NULL, MAVLINK_TYPE_UINT8_T, 0, 8, offsetof(mavlink_fence_point_t, idx) }, \
         { "count", NULL, MAVLINK_TYPE_UINT8_T, 0, 9, offsetof(mavlink_fence_point_t, count) }, \
         } \
}
						       ...
						       uint8_t idx, uint8_t count, float lat, float lng)
{
	...
	return mavlink_finalize_message(msg, system_id, component_id, 10, 18);
}
						           ...
						           uint8_t idx,uint8_t count,float lat,float lng)
{
	...
	msg->msgid = MAVLINK_MSG_ID_FENCE_POINT;
	return mavlink_finalize_message_chan(msg, system_id, component_id, chan, 10, 18);
}
...
{
	return mavlink_msg_fence_point_pack(system_id, component_id, msg, fence_point->idx, fence_point->count, fence_point->lat, fence_point->lng);
}
...
static inline void mavlink_msg_fence_point_send(mavlink_channel_t chan, uint8_t idx, uint8_t count, float lat, float lng)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[10];
	_mav_put_float(buf, 0, lat);
	...
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_FENCE_POINT, buf, 10, 18);
#else
	mavlink_fence_point_t packet;
	packet.lat = lat;
	packet.lng = lng;
	...
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_FENCE_POINT, (const char *)&packet, 10, 18);
#endif
}
...
#else
	memcpy(fence_point, _MAV_PAYLOAD(msg), 10);
#endif
}
