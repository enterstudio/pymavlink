...
typedef struct __mavlink_mission_ack_t
{
 ...
 byte type; ///< See MAV_MISSION_RESULT enum
...
static inline void mavlink_msg_mission_ack_send(mavlink_channel_t chan, byte target_system, byte target_component, byte type)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[3];
	_mav_put_byte(buf, 0, target_system);
 ...
 * @return System ID
 */
static inline byte mavlink_msg_mission_ack_get_target_system(const mavlink_message_t* msg)
{
 ...
 * @return Component ID
 */
static inline byte mavlink_msg_mission_ack_get_target_component(const mavlink_message_t* msg)
{
 ...
 */
static inline byte mavlink_msg_mission_ack_get_type(const mavlink_message_t* msg)
{
