...
static inline uint16_t mavlink_msg_debug_vect_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       const string *name, UInt64 time_usec, Single x, Single y, Single z)
{
...
static inline void mavlink_msg_debug_vect_send(mavlink_channel_t chan, const string *name, UInt64 time_usec, Single x, Single y, Single z)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[30];
	_mav_put_UInt64(buf, 0, time_usec);
...
{
	return _MAV_RETURN_string_array(msg, name, 10,  20);
}
 ...
 * @return Timestamp
 */
static inline UInt64 mavlink_msg_debug_vect_get_time_usec(const mavlink_message_t* msg)
{
