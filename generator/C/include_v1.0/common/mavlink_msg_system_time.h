...
{
 UInt64 time_unix_usec; ///< Timestamp of the master clock in microseconds since UNIX epoch.
 UInt32 time_boot_ms; ///< Timestamp of the component clock since boot time in milliseconds.
...
static inline void mavlink_msg_system_time_send(mavlink_channel_t chan, UInt64 time_unix_usec, UInt32 time_boot_ms)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[12];
	_mav_put_UInt64(buf, 0, time_unix_usec);
 ...
 */
static inline UInt64 mavlink_msg_system_time_get_time_unix_usec(const mavlink_message_t* msg)
{
 ...
 */
static inline UInt32 mavlink_msg_system_time_get_time_boot_ms(const mavlink_message_t* msg)
{
