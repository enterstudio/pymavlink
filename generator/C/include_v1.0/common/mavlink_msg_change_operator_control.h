...
{
 byte target_system; ///< System the GCS requests control for
 byte control_request; ///< 0: request control of this MAV, 1: Release control of this MAV
 byte version; ///< 0: key as plaintext, 1-255: future, different hashing/encryption variants. The GCS should in general use the safest mode possible initially and then gradually move down the encryption level if it gets a NACK message indicating an encryption mismatch.
 string passkey[25]; ///< Password / Key, depending on version plaintext or encrypted. 25 or less characters, NULL terminated. The characters may involve A-Z, a-z, 0-9, and "!?,.-"
...
static inline uint16_t mavlink_msg_change_operator_control_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       byte target_system, byte control_request, byte version, const string *passkey)
{
	...
	mav_array_memcpy(packet.passkey, passkey, sizeof(string)*25);
...
static inline void mavlink_msg_change_operator_control_send(mavlink_channel_t chan, byte target_system, byte control_request, byte version, const string *passkey)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[28];
	_mav_put_byte(buf, 0, target_system);
 ...
 */
static inline byte mavlink_msg_change_operator_control_get_target_system(const mavlink_message_t* msg)
{
 ...
 */
static inline byte mavlink_msg_change_operator_control_get_control_request(const mavlink_message_t* msg)
{
 ...
 */
static inline byte mavlink_msg_change_operator_control_get_version(const mavlink_message_t* msg)
{
 ...
 */
static inline uint16_t mavlink_msg_change_operator_control_get_passkey(const mavlink_message_t* msg, string *passkey)
{
	return _MAV_RETURN_string_array(msg, passkey, 25,  3);
}
