...
{
 UInt16 address; ///< Starting address of the debug variables
 byte ver; ///< Version code of the type variable. 0=unknown, type ignored and assumed int16_t. 1=as below
 byte type; ///< Type code of the memory variables. for ver = 1: 0=16 x int16_t, 1=16 x uint16_t, 2=16 x Q15, 3=16 x 1Q14
 byte[] value[32]; ///< Memory contents at specified address
...
static inline uint16_t mavlink_msg_memory_vect_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       UInt16 address, byte ver, byte type, const byte[] *value)
{
	...
	packet.ver = ver;
	packet.type = type;
	mav_array_memcpy(packet.value, value, sizeof(byte[])*32);
...
static inline void mavlink_msg_memory_vect_send(mavlink_channel_t chan, UInt16 address, byte ver, byte type, const byte[] *value)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[36];
	_mav_put_UInt16(buf, 0, address);
 ...
 */
static inline UInt16 mavlink_msg_memory_vect_get_address(const mavlink_message_t* msg)
{
 ...
 */
static inline byte mavlink_msg_memory_vect_get_ver(const mavlink_message_t* msg)
{
 ...
 */
static inline byte mavlink_msg_memory_vect_get_type(const mavlink_message_t* msg)
{
 ...
 */
static inline uint16_t mavlink_msg_memory_vect_get_value(const mavlink_message_t* msg, byte[] *value)
{
	return _MAV_RETURN_byte[]_array(msg, value, 32,  4);
}
