...
typedef struct __mavlink_statustext_t
{
 ...
 string text[50]; ///< Status text message, without null termination character
...
static inline uint16_t mavlink_msg_statustext_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       byte severity, const string *text)
{
	...
	mav_array_memcpy(packet.text, text, sizeof(string)*50);
...
static inline void mavlink_msg_statustext_send(mavlink_channel_t chan, byte severity, const string *text)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[51];
	_mav_put_byte(buf, 0, severity);
 ...
 */
static inline uint16_t mavlink_msg_statustext_get_text(const mavlink_message_t* msg, string *text)
{
	return _MAV_RETURN_string_array(msg, text, 50,  1);
}
