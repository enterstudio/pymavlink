						       ...
						       byte target_system, byte target_component, byte session, byte zoom_pos, byte zoom_step, byte focus_lock, byte shot, byte command_id, byte extra_param, Single extra_value)
{
	...
	_mav_put_byte(buf, 2, session);
	_mav_put_byte(buf, 3, zoom_pos);
	_mav_put_byte(buf, 4, zoom_step);
	_mav_put_byte(buf, 5, focus_lock);
	_mav_put_byte(buf, 6, shot);
	_mav_put_byte(buf, 7, command_id);
	_mav_put_byte(buf, 8, extra_param);
	_mav_put_Single(buf, 9, extra_value);
