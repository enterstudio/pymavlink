						       ...
						       byte target_system, byte target_component, byte mode, UInt16 shutter_speed, byte aperture, byte iso, byte exposure_type, byte command_id, byte engine_cut_off, byte extra_param, Single extra_value)
{
	...
	_mav_put_UInt16(buf, 3, shutter_speed);
	_mav_put_byte(buf, 5, aperture);
	_mav_put_byte(buf, 6, iso);
	_mav_put_byte(buf, 7, exposure_type);
	_mav_put_byte(buf, 8, command_id);
	_mav_put_byte(buf, 9, engine_cut_off);
	_mav_put_byte(buf, 10, extra_param);
	_mav_put_Single(buf, 11, extra_value);
