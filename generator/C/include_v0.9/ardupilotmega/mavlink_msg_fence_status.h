...
{
 byte breach_status; ///< 0 if currently inside fence, 1 if outside
 UInt16 breach_count; ///< number of fence breaches
 byte breach_type; ///< last breach type (see FENCE_BREACH_* enum)
 UInt32 breach_time; ///< time of last breach in milliseconds since boot
...
static inline uint16_t mavlink_msg_fence_status_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       byte breach_status, UInt16 breach_count, byte breach_type, UInt32 breach_time)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[8];
	_mav_put_byte(buf, 0, breach_status);
	_mav_put_UInt16(buf, 1, breach_count);
	_mav_put_byte(buf, 3, breach_type);
	_mav_put_UInt32(buf, 4, breach_time);
...
static inline void mavlink_msg_fence_status_send(mavlink_channel_t chan, byte breach_status, UInt16 breach_count, byte breach_type, UInt32 breach_time)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[8];
	_mav_put_byte(buf, 0, breach_status);
 ...
 */
static inline byte mavlink_msg_fence_status_get_breach_status(const mavlink_message_t* msg)
{
 ...
 */
static inline UInt16 mavlink_msg_fence_status_get_breach_count(const mavlink_message_t* msg)
{
 ...
 */
static inline byte mavlink_msg_fence_status_get_breach_type(const mavlink_message_t* msg)
{
 ...
 */
static inline UInt32 mavlink_msg_fence_status_get_breach_time(const mavlink_message_t* msg)
{
