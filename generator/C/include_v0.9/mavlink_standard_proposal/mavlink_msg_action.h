...
#define MAVLINK_MESSAGE_INFO_ACTION { \
	"ACTION", \
	3, \
	{  { "target_system", NULL, MAVLINK_TYPE_UINT8_T, 0, 0, offsetof(mavlink_action_t, target_system) }, \
...
static inline uint16_t mavlink_msg_action_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       byte target_system, byte target_component, byte action)
{
...
static inline void mavlink_msg_action_send(mavlink_channel_t chan, byte target_system, byte target_component, byte action)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[3];
	_mav_put_byte(buf, 0, target_system);
 ...
 */
static inline byte mavlink_msg_action_get_target_system(const mavlink_message_t* msg)
{
...
static inline void mavlink_msg_action_decode(const mavlink_message_t* msg, mavlink_action_t* action)
{
	...
	action->target_system = mavlink_msg_action_get_target_system(msg);
