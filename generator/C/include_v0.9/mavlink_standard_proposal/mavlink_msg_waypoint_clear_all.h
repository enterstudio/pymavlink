...
#define MAVLINK_MSG_ID_18_LEN 2
...
static inline void mavlink_msg_waypoint_clear_all_send(mavlink_channel_t chan, byte target_system, byte target_component)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[2];
	_mav_put_byte(buf, 0, target_system);
 ...
 * @return System ID
 */
static inline byte mavlink_msg_waypoint_clear_all_get_target_system(const mavlink_message_t* msg)
{
 ...
 * @return Component ID
 */
static inline byte mavlink_msg_waypoint_clear_all_get_target_component(const mavlink_message_t* msg)
{
