...
typedef struct __mavlink_gps_raw_t
{
 ...
 Single alt; ///< Altitude in meters
 Single eph; ///< GPS HDOP
 Single epv; ///< GPS VDOP
 Single v; ///< GPS ground speed
...
static inline void mavlink_msg_gps_raw_send(mavlink_channel_t chan, UInt64 usec, byte fix_type, Single lat, Single lon, Single alt, Single eph, Single epv, Single v, Single hdg)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[37];
	_mav_put_UInt64(buf, 0, usec);
 ...
 */
static inline UInt64 mavlink_msg_gps_raw_get_usec(const mavlink_message_t* msg)
{
