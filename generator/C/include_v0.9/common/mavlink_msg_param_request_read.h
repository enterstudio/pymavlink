...
static inline uint16_t mavlink_msg_param_request_read_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       byte target_system, byte target_component, const byte[] *param_id, Int16 param_index)
{
...
static inline void mavlink_msg_param_request_read_send(mavlink_channel_t chan, byte target_system, byte target_component, const byte[] *param_id, Int16 param_index)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[19];
	_mav_put_byte(buf, 0, target_system);
...
{
	return _MAV_RETURN_Int16(msg,  17);
}
