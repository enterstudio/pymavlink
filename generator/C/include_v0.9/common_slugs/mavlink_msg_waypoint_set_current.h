...
static inline void mavlink_msg_waypoint_set_current_send(mavlink_channel_t chan, byte target_system, byte target_component, UInt16 seq)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[4];
	_mav_put_byte(buf, 0, target_system);
 ...
 * @return Sequence
 */
static inline UInt16 mavlink_msg_waypoint_set_current_get_seq(const mavlink_message_t* msg)
{
