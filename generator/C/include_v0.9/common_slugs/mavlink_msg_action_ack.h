...
typedef struct __mavlink_action_ack_t
{
 ...
 byte result; ///< 0: Action DENIED, 1: Action executed
...
static inline void mavlink_msg_action_ack_send(mavlink_channel_t chan, byte action, byte result)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[2];
	_mav_put_byte(buf, 0, action);
 ...
 */
static inline byte mavlink_msg_action_ack_get_action(const mavlink_message_t* msg)
{
 ...
 */
static inline byte mavlink_msg_action_ack_get_result(const mavlink_message_t* msg)
{
