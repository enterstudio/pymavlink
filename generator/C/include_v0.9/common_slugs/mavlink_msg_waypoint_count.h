...
typedef struct __mavlink_waypoint_count_t
{
 ...
 UInt16 count; ///< Number of Waypoints in the Sequence
...
static inline void mavlink_msg_waypoint_count_send(mavlink_channel_t chan, byte target_system, byte target_component, UInt16 count)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[4];
	_mav_put_byte(buf, 0, target_system);
 ...
 * @return System ID
 */
static inline byte mavlink_msg_waypoint_count_get_target_system(const mavlink_message_t* msg)
{
 ...
 * @return Component ID
 */
static inline byte mavlink_msg_waypoint_count_get_target_component(const mavlink_message_t* msg)
{
 ...
 */
static inline UInt16 mavlink_msg_waypoint_count_get_count(const mavlink_message_t* msg)
{
